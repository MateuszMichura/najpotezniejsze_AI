...
function sendWelcomeMessage() {
    bot.chat("Cześć! Jestem GroqBot, asystent Minecraft. Możesz komunikować się ze mną naturalnym językiem.");
    bot.chat("Przykłady: 'Co mogę skraftować?', 'Przetop 5 żelaza', 'Zaatakuj najbliższego zombie', 'Zbierz pobliskie przedmioty'");
}
...
async function collectBlock(blockType, num = 1) {
    try {
        ...
        for (const blockPos of blocks) {
            ...
            await bot.pathfinder.goto(new GoalNear(blockPos.x, blockPos.y, blockPos.z, 1));
...
async function craftRecipe(bot, itemName, num = 1) {
    ...
    let recipes = bot.recipesFor(bot.registry.itemsByName[itemName].id, null, 1, null);
    ...
    if (!recipes || recipes.length === 0) {
        ...
        if (craftingTable === null) {
            ...
            if (hasTable) {
                ...
                if (craftingTable) {
                    recipes = bot.recipesFor(bot.registry.itemsByName[itemName].id, null, 1, craftingTable);
                    placedTable = true;
                }
            }
            else {
                log(bot, `You either do not have enough resources to craft ${itemName} or it requires a crafting table.`, true);
                return false;
            }
        }
        else {
    ...
    log(bot, `Successfully crafted ${itemName}, you now have ${bot.inventory.count(itemName)} ${itemName}.`, true);
    if (placedTable) {
        await collectBlock('crafting_table', 1);
    }
    return true;
...
async function smeltItem(bot, itemName, num=1) {
    ...
    if (!itemName.includes('raw') && !foods.includes(itemName)) {
        log(bot, `Cannot smelt ${itemName}, must be a "raw" item, like "raw_iron".`, true);
        return false;
    }
    ...
    if (!furnaceBlock){
        let hasFurnace = bot.inventory.items().some(item => item.name === 'furnace');
        if (hasFurnace) {
            let pos = bot.entity.position.offset(1, 0, 0);
            await bot.placeBlock(bot.blockAt(pos), new Vec3(0, 1, 0));
            furnaceBlock = bot.findBlock({
    ...
    if (!furnace.fuelItem()) {
        ...
        if (!fuel || fuel.count < put_fuel) {
            log(bot, `You do not have enough coal or charcoal to smelt ${num} ${itemName}, you need ${put_fuel} coal or charcoal`, true);
            if (placedFurnace)
                await collectBlock('furnace', 1);
            return false;
        }
    ...
    log(bot, `Successfully smelted ${itemName}, got ${total} ${smelted_item.name}.`, true);
    return true;
}
...
async function clearNearestFurnace(bot) {
    ...
    log(bot, `Cleared furnace, received ${smelted_name}, ${input_name}, and ${fuel_name}.`, true);
    return true;
}
...
async function attackNearestMob(bot) {
    ...
    try {
        ...
        return { success: false, error: `Nie udało się zaatakować moba: ${error.message}` };
    }
}
...
async function defendSelf(bot, range=9) {
    ...
    while (enemy) {
        if (bot.entity.position.distanceTo(enemy.position) > 3) {
            try {
                await bot.pathfinder.goto(new GoalNear(enemy.position.x, enemy.position.y, enemy.position.z, 2));
            } catch (err) {
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        enemy = bot.nearestEntity(entity => 
            entity.type === 'mob' && 
            bot.entity.position.distanceTo(entity.position) < range &&
            entity.name !== 'Armor Stand'
        );
...
async function pickupNearbyItems(bot) {
    ...
    while (nearestItem) {
        ...
        await bot.pathfinder.goto(new GoalNear(nearestItem.position.x, nearestItem.position.y, nearestItem.position.z, 1));
        await new Promise(resolve => setTimeout(resolve, 200));
...
bot.on('chat', async (username, message) => {
    ...
    try {
        ...
        const response = await axios.post(GROQ_API_ENDPOINT, {
            messages: [
                {
                    role: "system",
                    ...
                    Dostępne do skraftowania przedmioty: ${availableRecipes.map(r => r.result.name).join(', ')}
            ...
            switch (command) {
                ...
                case 'zbierz':
                    if (args.length >= 2) {
                        ...
                        bot.chat("Niepoprawna komenda zbierania. Użyj: !zbierz [liczba] [typ_bloku]");
                    }
                    break;
                case 'kraftuj':
                    if (args.length >= 2) {
                        ...
                        bot.chat("Niepoprawna komenda kraftowania. Użyj: !kraftuj [przedmiot] [liczba]");
                    }
                    break;
                case 'lista_kraftowania':
                    const recipes = await bot.recipesAll(null, null, null);
                    bot.chat(`Mogę skraftować: ${recipes.map(r => r.result.name).join(', ')}`);
                    break;
                ...
                case 'przetop':
                    if (args.length >= 2) {
                        ...
                        bot.chat("Niepoprawna komenda przetapiania. Użyj: !przetop [przedmiot] [liczba]");
                    }
                    break;
                case 'wyczyść_piec':
        ...
        console.error('Błąd komunikacji z API Groq:', error.response ? error.response.data : error.message);
...
console.log('Bot Minecraft z integracją Groq i wszystkimi funkcjonalnościami jest uruchomiony...');